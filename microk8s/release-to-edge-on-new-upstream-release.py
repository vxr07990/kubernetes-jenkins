...
if __name__ == '__main__':
    ...
    for track in tracks:
        ...
        if upstream != edge_snap.version:
            ...
            '''
            if minor_version_difference(upstream, snapped):
                # There is a minor version difference. This should occur only in the "latest" track
                # when a new stable release is available upstream eg from 1.11 to 1.12
                # we need to
                # 1. branch the code
                # 2. update the old lp builders to use the new branch
                # 3. create lp builders for the new upstream "track" (pointing to the master branch)
                if track != 'latest':
                    raise Exception("Tracks should not change releases")
                create_branch(snapped)
                point_builders_to(snapped)
                create_builder_for_new_track(upstream)
            '''
