...
from utils import run_bundletester, conjureup, juju_deploy
...
async def test_deploy(bundle, log_dir):
    async with temporary_model(log_dir) as model:
        done_deployment = await conjureup(model, namespace, bundle, charm_channel,
                                          snap_channel)
        if not done_deployment:
...
async def test_upgrade(bundle, log_dir):
    async with temporary_model(log_dir) as model:
        done_deployment = await conjureup(model, namespace, bundle, 'stable')
        if not done_deployment:
